...
CJK languages (ja-JP, ko-KR, zh-CN, zh-TW) are **not** supported in
v1 — the romanizer modules need the modern 2-suffixed ECI API rather
than the legacy one v1 uses. The dylibs ship in the vendor tree and
convert cleanly, but `sd_eloquence` refuses to load them. v2 will
switch the engine wrapper to the 2-API and re-enable them; background
in `docs/cjk-investigation/` and `docs/eci-2-api/`.
